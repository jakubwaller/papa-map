...
patterns: `the author's `beer-map` repo` (same author, same deploy target). Project context:
`the private prior-art notes` (esp. "Design
...
Self-contained HTML (minimal inline CSS, no JS deps). Content (source from the private research
