...
# The area index behind the map's footer link (pipeline/pages.area_index):
# one row per generated area page with its bbox, so the link can follow the
# map view — Hamburg when zoomed into Hamburg, Deutschland at country zoom,
# Danmark after a pan north — instead of the UI language (2026-09-17). Shape
# in CONTRACT.md v32. Under /data/ like the other nightly JSON, same mount.
AREAS_PATH = os.environ.get("PAPAMAP_AREAS_PATH", "web/data/areas.json")
